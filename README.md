...
- `/start` — для непривязанного пользователя объяснение Turbo Rating и три
  ...
  количество Turbo и WR, последние пять результатов W/L и время успешного sync.
  Время показано в UTC и хранится в памяти процесса; после перезапуска до первого
  успешного sync показано «Обновлено: ожидаем обновления».
...
- `/profile` — игрок, Dota ID, Turbo Rating, место, количество Turbo-игр
  и winrate после регистрации.
...
- `/rating` — текущий и стартовый рейтинг, рекорд TR, изменение за 7 дней,
  место, статистика Turbo и общая разница. Подробные начисления открываются
  кнопкой «📈 История TR» или командой `/history`; история не пересчитывается.
  Также показаны последние пять завершённых Turbo (новейший слева) и текущая серия
  побед или поражений, в том числе длиннее пяти игр. Калибровка, другие режимы
  и неизвестные результаты исключены. Серия не влияет на рейтинг.
- `/history` — текущий и рекордный TR, изменения за сегодня, 7 и 30 дней,
  текущее место и позиции 7/30 дней назад, максимум 10 последних начислений
  с датой, изменением и рейтингом после него.
...
«ℹ️ Как считается рейтинг» доступна до и после регистрации и объясняет
...
«🔗 Поделиться» отправляет короткое приглашение со ссылкой на текущего бота;
...
При запуске бот регистрирует меню `/start`, `/add`, `/rating`, `/history`, `/stats`, `/top`,
`/matches`, `/sync`, `/profile`. Старые подписи «🏆 Рейтинг» и «🥇 Топ» ещё работают.
...
Нажмите «🔁 Сменить Dota». Бот покажет текущий nickname и Dota ID, затем
