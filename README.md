...
- `/rating` — текущий и стартовый рейтинг, место, статистика Turbo, разница и последние
...
При запуске бот регистрирует меню `/start`, `/add`, `/rating`, `/stats`, `/top`,
...
до последнего `rating_after`. Для одной игры — «🟢 Победа · Phantom Assassin»,
изменение TR и рейтинг до/после. В конце уведомления показано изменение места,
например «Место: #4 → #3», либо только текущее место, если оно не изменилось.
...
```powershell
.\.venv\Scripts\python.exe -X utf8 -m unittest scripts.test_rating scripts.test_autosync scripts.test_ux scripts.test_accounts -v
```
