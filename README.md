...
- `/start` — для непривязанного пользователя объяснение Turbo Rating и три
  кнопки: «➕ Привязать Dota», «🥇 Посмотреть топ», «ℹ️ Как считается рейтинг».
  Для зарегистрированного — компактный главный экран: nickname, рейтинг и место,
  последние пять результатов W/L (новейший слева). Калибровка, другие режимы
  и неизвестные результаты исключены.
...
- `/profile` — nickname, Dota ID, дата подключения и одна строка текущего TR.
...
- `/rating` — текущий TR, место, стартовый и рекордный TR, изменение за 7 дней.
...
«ℹ️ Как считается рейтинг» объясняет
стартовый рейтинг, влияние побед и поражений без формул и технических деталей.
Старая подпись этой кнопки обрабатывается и после регистрации.
...
Кнопки используют те же обработчики, что и команды. `/add` без аргумента
...
и «🥇 Топ» ещё работают; `/stats` доступна без отдельного пункта меню команд.
...
`/matches` читает SQLite. `/profile`, `/rating`, `/history` и `/stats` инициализируют отсутствующий
